...
class Collection1TabContent extends StatefulWidget {
  ...
  @override
  _Collection1TabContentState createState() => _Collection1TabContentState();
}

class _Collection1TabContentState extends State<Collection1TabContent> {
  List<PoemData> _collection1Results = [];
  int _currentPage = 1;
  ...
  Future<void> _loadData() async {
    ...
    try {
      ...
      List<PoemData> newResults = results
          .where((result) => result.type == 'collection1')
          .map((result) => result.poemData!)
          .toList();

      setState(() {
        _currentPage++;
        _collection1Results.addAll(newResults);
        _isLoading = false;
  ...
  Widget build(BuildContext context) {
    if (_collection1Results.isEmpty && !_hasMoreData) {
      return Center(
        child: Text(
          '未找到与 "${widget.searchQuery}" 相关的作品集',
    ...
    return ListView.builder(
      ...
      itemCount: _collection1Results.length + 1,
      itemBuilder: (context, index) {
        if (index == _collection1Results.length) {
          if (_hasMoreData) {
        ...
        final poem = _collection1Results[index];
