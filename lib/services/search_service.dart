  ...
  static Future<List<dynamic>> search({
    ...
    if (option == '作者') {
      ...
      for (var map in collection1Result) {
        PoemData poemData = PoemData.fromMap(map);
        results.add(GlobalSearchResult.poem(poemData)); // 或者根据需要创建不同的类型
      }
      ...
      for (var map in collection2Result) {
        ...
        results.add(GlobalSearchResult.collection(collectionData));
      }
      ...
      for (var map in sentenceResult) {
        ...
        results.add(GlobalSearchResult.sentence(sentenceData));
      }
